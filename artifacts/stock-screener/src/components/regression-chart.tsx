...
function StatBadge({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-center bg-zinc-800/60 rounded-lg px-4 py-2 border border-zinc-700/50">
...
export function RegressionChart() {
  ...
  // thin out points for very long periods to keep render snappy
    ...
    <section className="mb-4 border border-zinc-500/70 rounded-xl p-4">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 justify-between">
        <div className="flex items-center gap-3">
          <h3 className="text-lg font-bold text-foreground">OLS Regression</h3>
          {data?.symbol && (
      ...
      {/* Annualised return colour override — shown inline */}
      {stats && (
        <div className="hidden">
          <span className={annRetColor} />
        </div>
      )}
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={displayData} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
              ...
              <Legend
                ...
                formatter={(value) => (
                  <span style={{ color: value === "Close" ? "#94a3b8" : "#ef4444" }}>{value}</span>
                )}
              />
              <Line
                type="monotone"
