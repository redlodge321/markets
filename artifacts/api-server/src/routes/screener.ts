...
function computeOLS(closes: number[]) {
  ...
  // Annualised return: daily slope as fraction of first fitted value × 252 trading days
  const firstFitted = intercept;
  const annualisedReturn = firstFitted !== 0 ? (slope / firstFitted) * 252 * 100 : 0;
...
router.get("/screener/regression", async (req, res) => {
  ...
  try {
    const rows = await yf.historical(symbol, {
      period1: periodToStartDate(period),
      interval: "1d",
    });
